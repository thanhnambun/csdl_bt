...
delimiter // 
    ...
    begin
        ...
        from customers
        where phoneNumber_in = phonenumber;
    end; 
    
// delimiter ;

delimiter //
		create procedure GetTotalBalance (in paramin int ,out totalbalance int)
		begin
			select CustomerID , sum(balance) into totalbalance
			from accounts c
			where CustomerID = paramin
...
delimiter // 
	    create procedure IncreaseEmployeeSalary(out salary_update decimal(10,2),in EmployeeID_in int  )
        begin
			update employees
            set salary =salary* 1.1
            where EmployeeID = EmployeeID_in ;
            SELECT salary INTO salary_update 
			FROM employees
			WHERE EmployeeID = EmployeeID_in;
        end;
// delimiter ;	
...
select totalbalance;
SET @salary = 0;
CALL IncreaseEmployeeSalary(@salary, 1);
SELECT @salary; 
